...
const filteredMachines = computed(() => {
    ...
    if (filterMode.value === 'online') {
        ...
        result = result.filter(m => machineStatus.value[m.id] === 'offline' || machineStatus.value[m.id] === 'unknown');
    }
...
const selectedOnlineCount = computed(() => {
    return Array.from(selectedMachines.value).filter(id => machineStatus.value[id] === 'online').length;
});
    ...
    <TooltipProvider>
        <div class="flex h-screen w-screen bg-canvas text-primary overflow-hidden">
            <aside class="app-sidebar" data-tauri-drag-region>
                <div class="h-14 flex items-center px-5 gap-3 font-semibold select-none border-b border-subtle"
            ...
            <main class="flex-1 flex flex-col bg-canvas overflow-hidden">
                <header class="app-header" data-tauri-drag-region>
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative">
                            <span class="icon-[lucide--search] size-4 absolute left-3 top-1/2 -translate-y-1/2 text-tertiary"></span>
                            <Input v-model="searchQuery" placeholder="搜索机器... (⌘F)" class="pl-9 w-48 h-9 app-input" />
                        </div>

                        <div class="flex items-center gap-1.5 bg-elevated rounded-lg px-2 py-1 border border-subtle">
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="ghost" size="sm" 
                                        :class="['px-2 h-7 text-xs', filterMode === 'all' && 'bg-accent/30 text-accent']"
                            ...
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="ghost" size="sm"
                                        :class="['px-2 h-7 text-xs', filterMode === 'online' && 'bg-success/20 text-success']"
                            ...
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="ghost" size="sm"
                                        :class="['px-2 h-7 text-xs', filterMode === 'offline' && 'bg-danger/20 text-danger']"
                        ...
                        <Tooltip>
                            <TooltipTrigger as-child>
                                <Button variant="ghost" size="icon-sm" @click="testAllConnections" :disabled="isLoading">
                                    <span class="icon-[lucide--wifi] size-4" :class="{ 'animate-pulse': isLoading }"></span>
                                </Button>
                            </TooltipTrigger>
                        ...
                        <Tooltip>
                            <TooltipTrigger as-child>
                                <Button variant="ghost" size="icon-sm" @click="loadMachines" :disabled="isLoading">
                                    <span class="icon-[lucide--refresh-cw] size-4" :class="{ 'animate-spin': isLoading }"></span>
                                </Button>
                            </TooltipTrigger>
                ...
                <div v-if="isSelectionMode" class="mx-6 mt-3 p-3 bg-elevated border border-accent/30 rounded-xl flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        <span class="text-sm text-secondary">已选择 {{ selectedCount }} 台机器</span>
                        <Button variant="ghost" size="sm" class="text-xs px-2" @click="filteredMachines.forEach(m => selectedMachines.add(m.id))">
                            全选当前
                        </Button>
                    ...
                    <div class="flex gap-2">
                        <Tooltip>
                            <TooltipTrigger as-child>
                                <Button variant="outline" size="sm" @click="testSelectedConnections" :disabled="selectedCount === 0">
                        ...
                        <Tooltip>
                            <TooltipTrigger as-child>
                                <Button variant="destructive" size="sm" @click="showBatchDeleteConfirm = true" :disabled="selectedCount === 0">
                ...
                <div v-if="showSkeleton && machines.length === 0"
                ...
                <div v-else-if="isLoading && machines.length === 0"
                ...
                <div v-else-if="machines.length === 0"
                    ...
                    <div class="size-20 rounded-2xl bg-active grid place-items-center mb-5 shadow-[0_10px_24px_rgba(0,0,0,0.30)]">
                ...
                <div v-else class="flex-1 overflow-y-auto p-6 space-y-6">
                    <div v-for="group in groupedMachines" :key="group.key">
                        <div v-if="groupMode !== 'none'" class="app-group-header mb-3">
                            ...
                            <Button v-if="isSelectionMode" variant="ghost" size="sm" 
                        ...
                        <div class="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-4">
                            <div v-for="machine in group.machines" :key="machine.id"
                                :class="[
                                ...
                                <div class="flex items-start gap-3 mb-3">
                                    ...
                                    <div v-else class="app-icon-container">
                                        <span v-if="machine.os === 'windows'" class="icon-[mdi--microsoft-windows] size-5 text-info"></span>
                                        <span v-else-if="machine.os === 'macos'" class="icon-[mdi--apple] size-5 text-secondary"></span>
                                        <span v-else class="icon-[mdi--linux] size-5 text-warning"></span>
                                    </div>

                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium truncate text-primary">{{ machine.name || machine.host }}</span>
                                            <Tooltip>
                                                <TooltipTrigger>
                                    ...
                                    <div v-if="!isSelectionMode" class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-120">
                                        <Tooltip>
                                            <TooltipTrigger as-child>
                                                <Button variant="ghost" size="icon-sm" class="app-btn-ghost"
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger as-child>
                                                <Button variant="ghost" size="icon-sm" class="app-btn-danger"
                                ...
                                <div v-if="parseTags(machine.tags).length > 0"
                                    ...
                                    <Badge v-for="tag in parseTags(machine.tags).slice(0, 3)" :key="tag" 
                                    ...
                                    <Tooltip v-if="parseTags(machine.tags).length > 3">
                                        ...
                                        <TooltipContent>
                                            <div class="flex gap-1">
                                                <Badge v-for="tag in parseTags(machine.tags).slice(3)" :key="tag" 
                                                    class="app-badge app-badge-secondary text-xs">
                                                    {{ tag }}
                                                </Badge>
                                            </div>
                                ...
                                <div class="flex justify-between items-center pt-3 border-t border-subtle">
                                    ...
                                    <Button v-if="!isSelectionMode" size="sm" 
            ...
            <Dialog v-model:open="showAddModal">
                <DialogContent class="sm:max-w-md bg-surface border border-subtle rounded-xl shadow-[0_24px_64px_rgba(0,0,0,0.55)]">
                    <DialogHeader>
                    ...
                    <div class="space-y-4 py-4">
                        ...
                        <div class="flex gap-3">
                            <div class="flex-1 space-y-1.5">
                                <label class="text-sm font-medium text-secondary">主机地址 <span class="text-danger">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-sm font-medium text-secondary">用户名 <span class="text-danger">*</span></label>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="flex gap-2">
                                <Button :variant="newMachine.authType === 'password' ? 'default' : 'outline'" size="sm"
                                    :class="['flex-1 app-btn', newMachine.authType === 'password' && 'app-btn-primary']"
                                    @click="newMachine.authType = 'password'">密码</Button>
                                <Button :variant="newMachine.authType === 'key' ? 'default' : 'outline'" size="sm"
                                    ...
                                    @click="newMachine.authType = 'key'">密钥</Button>
                            </div>
                        </div>

                        <div v-if="newMachine.authType === 'password'" class="space-y-1.5">
                            <label class="text-sm font-medium text-secondary">密码</label>
                            <Input v-model="newMachine.password" type="password" placeholder="SSH 密码" class="app-input" />
                        </div>
                        <div v-else class="space-y-1.5">
                            <label class="text-sm font-medium text-secondary">私钥路径</label>
                            <Input v-model="newMachine.privateKeyPath" placeholder="~/.ssh/id_rsa" class="app-input" />
                        </div>
            ...
            <Dialog v-model:open="showEditModal">
                <DialogContent v-if="editingMachine" 
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="flex gap-2">
                                <Button :variant="editingMachine.authType === 'password' ? 'default' : 'outline'"
                                    size="sm" 
                                    :class="['flex-1 app-btn', editingMachine.authType === 'password' && 'app-btn-primary']"
                                    @click="editingMachine.authType = 'password'">密码</Button>
                                <Button :variant="editingMachine.authType === 'key' ? 'default' : 'outline'"
                                    size="sm"
                                    :class="['flex-1 app-btn', editingMachine.authType === 'key' && 'app-btn-primary']"
                                    @click="editingMachine.authType = 'key'">密钥</Button>
                            </div>
                        </div>
                        <div v-if="editingMachine.authType === 'password'" class="space-y-1.5">
                            <label class="text-sm font-medium text-secondary">密码</label>
                            <Input v-model="editingMachine.password" type="password" placeholder="留空保持不变" class="app-input" />
                        </div>
                        <div v-else class="space-y-1.5">
                            <label class="text-sm font-medium text-secondary">私钥路径</label>
                    ...
                    <DialogFooter>
                        ...
                        <Button class="app-btn app-btn-primary"
                            @click="handleUpdateMachine">保存</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            <Dialog v-model:open="showDeleteConfirm">
                <DialogContent v-if="deletingMachine" 
                    ...
                    <DialogHeader class="text-center">
                        ...
                        <DialogDescription class="text-secondary">
                            确定删除 <strong class="text-primary">{{ deletingMachine.name || deletingMachine.host }}</strong>？此操作不可恢复。
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter class="sm:justify-center gap-3">
                        <Button variant="outline" class="app-btn app-btn-secondary"
                            @click="showDeleteConfirm = false">取消</Button>
                        <Button variant="destructive" class="app-btn"
                            @click="handleDeleteMachine">删除</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog v-model:open="showBatchDeleteConfirm">
                <DialogContent 
                    ...
                    <DialogHeader class="text-center">
                        ...
                        <DialogDescription class="text-secondary">
                            确定删除选中的 <strong class="text-danger">{{ selectedCount }}</strong> 台机器？此操作不可恢复。
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter class="sm:justify-center gap-3">
                        <Button variant="outline" class="app-btn app-btn-secondary"
                            @click="showBatchDeleteConfirm = false">取消</Button>
                        <Button variant="destructive" class="app-btn"
                            @click="handleBatchDelete">删除</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>

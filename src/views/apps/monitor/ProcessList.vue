...
function getStatusClass(status: string): string {
  ...
  switch (firstChar) {
    ...
    default:
      return "bg-white/10 text-foreground/60";
  }
}
...
<template>
  <div class="h-full flex flex-col bg-card/50 rounded-2xl border border-border/50 overflow-hidden">
    ...
    <div
      class="flex gap-2 px-4 py-2.5 bg-muted/30 text-[0.7rem] text-muted-foreground uppercase tracking-wider border-b border-border/30"
    >
      <span
        v-for="col in columns"
        :key="col.key"
        class="shrink-0 cursor-pointer select-none hover:text-foreground/80 transition-colors"
    ...
    <div class="flex-1 overflow-auto">
      ...
      >
        <span class="text-muted-foreground font-mono text-xs w-[80px] shrink-0">{{
          proc.pid
        }}</span>
        <span
          class="flex items-center gap-2 font-medium flex-[2] min-w-0 truncate"
          :title="proc.command"
        >
          <span
            class="w-[22px] h-[22px] bg-gradient-to-br from-sky-400/30 to-sky-400/10 rounded-md flex items-center justify-center text-xs font-semibold text-sky-400 shrink-0"
            >{{ proc.name.charAt(0).toUpperCase() }}</span
          >
          {{ proc.name }}
        </span>
        ...
        <span class="w-[80px] shrink-0">
          <span
            class="inline-block px-1.5 py-0.5 rounded text-[0.65rem] font-medium"
        ...
        <span
          class="w-[70px] shrink-0 font-mono text-right"
          :class="proc.cpu > 50 ? 'text-amber-500 font-medium' : ''"
          >{{ proc.cpu.toFixed(1) }}%</span
        >
        <span
          class="w-[70px] shrink-0 font-mono text-right"
          :class="proc.memory > 50 ? 'text-amber-500 font-medium' : ''"
        ...
        <span class="flex gap-1 justify-end">
          <button
            class="flex items-center justify-center w-6 h-6 rounded-md bg-red-500/10 text-red-500 opacity-0 group-hover:opacity-100 hover:bg-red-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title="终止进程"
          ...
          <button
            class="flex items-center justify-center w-6 h-6 rounded-md bg-amber-500/10 text-amber-500 opacity-0 group-hover:opacity-100 hover:bg-amber-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title="强制终止"
            :disabled="killingPid === proc.pid"
      ...
      <div
        ...
        class="flex flex-col items-center justify-center py-10 text-muted-foreground/50 gap-3"
      >

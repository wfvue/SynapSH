...
const canConnect = computed(() => {
  return (
    host.value &&
    username.value &&
    (authType.value === "password" ? password.value : privateKey.value)
  );
});
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="p-4 flex flex-col gap-4">
      ...
      <div
        v-if="error"
        class="px-3 py-2.5 bg-destructive/10 border border-destructive rounded-md text-destructive text-sm"
      >
        {{ error }}
      </div>
